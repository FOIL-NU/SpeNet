...
clear
clc;
fnamein=sprintf('tubulin+CF660-SR10nm 2018 April 26 17_00_42.spe');
...
fnameout=sprintf('Alexa546_Tubulin_narrow 2018 March 22 11_31_52.dat');
img=A(:,67:83,:);
...
TR0=importdata('tubulin+CF660-SR10nm 2018 April 26 17_00_42.csv');
TR0=TR0.data;

TR=TR0(:,[3,4,2,6]); % x,y,frame number and intensity
...
w=2;%half width of the calculated region
...
%Live display
    if live==1
        ...
        subplot('Position',[0.3 0.1 0.65 0.35])
            ...
            % hold on
            % plot(smooth(bs(n,:),20),'r');hold off
            axis([151 size(bs,2) -100 4000])
%             set(gca,'XTick',[1:(size(bs,2)-1)/(size(fy,2)-1):size(bs,2)])
            set(gca,'XTick',[151:50:301]);;
            set(gca,'XTicklabel',[650:50:800]);
        %     set(gca,'YTick',[]); 
        drawnow
    end
...
fy = [500:25:750];%for xtick in plot
...
% set(gca,'xTick',[1:(size(spe,2)-1)/(size(fy2,2)-1):size(spe,2)]);
% set(gca,'xTicklabel',fy);
